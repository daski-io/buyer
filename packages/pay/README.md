...
Pending sponsorship keeps the preparation and signature for `--resume`; a gateway refusal raised before any sponsorship is reserved (`CONFIRMATION_SPONSORSHIP_LIMIT`, an invalid or stale preparation) clears it so `--submission direct` can proceed, while an ambiguous failure keeps it. A direct submission is tracked in the order store as `prepared`, `submitted` (a hash recorded, unverified), `observed`, or `abandoned`. `observed` requires the receipt to have succeeded in the chain's canonical block at its height, the pinned EAS to have emitted an event with the payer as attester whose attestation binds to the prepared call (every event in a batched receipt is tried), and the gateway's finalized read to be at or past the receipt's block, canonical at its height on the profile's RPC, and to show the result. A new preparation is refused while one is prepared or submitted. One order's record is updated under a per-order lock and every write to the order file under a store-wide lock; a lock is reclaimed only when its owner process is gone, never by age. A hash recorded by mistake is replaced (`--tx`) or cleared (`--abandon`) only once its transaction is canonical and finalized and no event in it binds to the prepared call (`DASKI_CONFIRMATION_TX_UNFINALIZED` until then); a missing, pending, or matching receipt keeps the record. Neither cancels anything at the wallet. Finality on Base takes minutes to tens of minutes.
